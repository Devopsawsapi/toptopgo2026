...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <form method="POST" action="<?php echo e(route('admin.sos.treat-all')); ?>"
            ...
            <button class="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-lg text-sm font-medium transition flex items-center gap-2">
                ✓ Tout marquer traité (<?php echo e($totalActive); ?>)
            </button>
        </form>
        <?php endif; ?>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 border-l-4 border-l-green-500">
            ...
            <div class="text-3xl font-bold text-green-600"><?php echo e($totalTreated); ?></div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 border-l-4 border-l-blue-500">
            ...
            <div class="text-3xl font-bold text-blue-600"><?php echo e($totalToday); ?></div>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <div class="text-sm text-gray-500 mb-1">Total</div>
            <div class="text-3xl font-bold text-gray-800"><?php echo e($totalAll); ?></div>
        </div>
    </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <?php if($totalActive > 0): ?>
                    <span class="bg-red-100 text-red-700 text-xs px-3 py-1 rounded-full font-medium animate-pulse">
                        🆘 <?php echo e($totalActive); ?> alerte(s) active(s)
                    </span>
                <?php endif; ?>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
        <form method="GET" action="<?php echo e(route('admin.sos.index')); ?>" class="flex gap-3 flex-wrap items-end">
            <div>
                ...
                <select name="status" class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500">
                    <option value="all"     <?php echo e(request('status') === 'all'     ? 'selected' : ''); ?>>Tous</option>
                    <option value="active"  <?php echo e(request('status') === 'active'  ? 'selected' : ''); ?>>🔴 Actives</option>
                    <option value="treated" <?php echo e(request('status') === 'treated' ? 'selected' : ''); ?>>✅ Traitées</option>
                </select>
            </div>
            <div>
                ...
                <select name="sender_type" class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500">
                    ...
                    <option value="driver" <?php echo e(request('sender_type') === 'driver' ? 'selected' : ''); ?>>🚗 Chauffeurs</option>
                    <option value="user"   <?php echo e(request('sender_type') === 'user'   ? 'selected' : ''); ?>>👤 Utilisateurs</option>
                </select>
            </div>
            <div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="divide-y divide-gray-50">
            <?php $__empty_1 = true; $__currentLoopData = $alerts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $alert): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                ...
                <div class="p-4 hover:bg-gray-50 transition <?php echo e($alert->status === 'active' ? 'border-l-4 border-l-red-500' : ''); ?>">
                    <div class="flex items-start justify-between gap-4">
                        ...
                        <div class="flex items-start gap-4 flex-1">
                            ...
                            <div class="w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold flex-shrink-0
                                <?php echo e($isDriver ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'); ?>">
                                <?php echo e($isDriver ? '🚗' : '👤'); ?>
                            ...
                            <div class="flex-1">
                                <div class="flex items-center gap-3 flex-wrap">
                                    <span class="font-semibold text-gray-800"><?php echo e($senderName); ?></span>
                                    <span class="text-xs px-2 py-0.5 rounded-full
                                        <?php echo e($isDriver ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'); ?>">
                                        <?php echo e($isDriver ? 'Chauffeur' : 'Utilisateur'); ?>
                                ...
                                <div class="flex items-center gap-4 mt-2 text-xs text-gray-400 flex-wrap">
                                    <span>🕐 <?php echo e($alert->created_at->format('d/m/Y H:i')); ?>

                                        (<?php echo e($alert->created_at->diffForHumans()); ?>)</span>
                                    <?php if($alert->trip_id): ?>
                                        <span>🚕 Course #<?php echo e($alert->trip_id); ?></span>
                                    <?php endif; ?>
                                    <?php if($alert->lat && $alert->lng): ?>
                                        <span>📍 <?php echo e(number_format($alert->lat, 4)); ?>, <?php echo e(number_format($alert->lng, 4)); ?></span>
                                    <?php endif; ?>
                                    <?php if($alert->status === 'treated' && $alert->treatedBy): ?>
                                        <span>✅ Traité par <?php echo e($alert->treatedBy->name ?? '—'); ?>

                                            le <?php echo e($alert->treated_at?->format('d/m/Y H:i')); ?></span>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2 flex-shrink-0">
                            <?php if($alert->lat && $alert->lng): ?>
                                <button onclick="zoomSos(<?php echo e($alert->lat); ?>, <?php echo e($alert->lng); ?>)"
                                    class="bg-blue-50 hover:bg-blue-100 text-blue-700 px-3 py-1.5 rounded-lg text-xs transition">
                                    📍 Localiser
                                </button>
                            <?php endif; ?>

                            <a href="<?php echo e(route('admin.sos.show', $alert->id)); ?>"
        ...
        <?php if($alerts->hasPages()): ?>
            <div class="p-4 border-t border-gray-100">
                <?php echo e($alerts->appends(request()->query())->links('pagination::tailwind')); ?>
...
function fetchSosAlerts() {
    fetch("<?php echo e(route('admin.sos.live')); ?>", {
...
<?php if($totalActive > 0): ?>
    document.title = '🆘 <?php echo e($totalActive); ?> SOS - TopTopGo Admin';
<?php endif; ?>
</script>
<?php $__env->stopPush(); ?>
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\SMART\Desktop\Nouveau dossier\Backendtoptopgo\Backendtoptopgo\resources\views/admin/sos/index.blade.php ENDPATH**/ ?>