...
<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
    ...
    <div class="bg-white p-6 rounded-2xl shadow-md border-l-4 border-blue-500
        ...
        <p class="text-green-500 text-sm mt-2">
            +<?php echo e(number_format($stats['new_users_today'] ?? 0)); ?> aujourd’hui
        </p>
    </div>
    ...
    <div class="bg-white p-6 rounded-2xl shadow-md border-l-4 border-yellow-500
        ...
        <h2 class="text-3xl font-bold text-yellow-500 mt-2">
            <?php echo e(number_format($stats['active_drivers'] ?? 0)); ?>

        </h2>
        <p class="text-gray-500 text-sm mt-2">
            <?php echo e(number_format($stats['online_drivers'] ?? 0)); ?> en ligne
        </p>
    </div>
    ...
    <div class="bg-white p-6 rounded-2xl shadow-md border-l-4 border-black
        ...
        <h2 class="text-3xl font-bold text-black mt-2">
            <?php echo e(number_format($stats['today_rides'] ?? 0)); ?>

        </h2>
        <p class="text-gray-500 text-sm mt-2">
            <?php echo e(number_format($stats['active_rides'] ?? 0)); ?> en cours
        </p>
    </div>
    ...
    <div class="bg-white p-6 rounded-2xl shadow-md border-l-4 border-green-500
        ...
        <h2 class="text-3xl font-bold text-green-500 mt-2">
            <?php echo e(number_format($stats['today_revenue'] ?? 0)); ?> XAF
        </h2>
        <p class="text-gray-500 text-sm mt-2">
            Commission : <?php echo e(number_format($stats['today_commission'] ?? 0)); ?> XAF
        </p>
    </div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    var drivers = <?php echo json_encode($drivers ?? [], 15, 512) ?>;
    var markers = [];
